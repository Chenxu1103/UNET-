"""
电缆缠绕检测 - 使用SimpleUNet模型（兼容旧checkpoint）

功能:
  - 检测电缆(cable)、胶带(tape)、毛刺(burr)
  - 计算胶带缠绕均匀性（通过直径比例）
  - 忽略其他缺陷类别（loose, bulge, damage等）
"""
...
class SimpleInference:
    ...
    def predict(self, frame_bgr: np.ndarray) -> np.ndarray:
        ...
        # 使用概率阈值而非argmax - 提高覆盖率
        # 电缆(1): 较低阈值，提高召回率
        cable_prob = probs_resized[1]
        cable_mask = (cable_prob >= 0.35).astype(np.uint8)

        # 胶带(2): 较低阈值，提高召回率
        tape_prob = probs_resized[2]
        tape_mask = (tape_prob >= 0.35).astype(np.uint8)
        ...
        if np.sum(cable_mask) > 0:
            ...
            cable_mask = cv2.dilate(cable_mask, kernel_small, iterations=1)

        # 胶带：先闭运算填充空洞，再膨胀扩展区域
        if np.sum(tape_mask) > 0:
            tape_mask = cv2.morphologyEx(tape_mask, cv2.MORPH_CLOSE, kernel_medium, iterations=2)
            tape_mask = cv2.dilate(tape_mask, kernel_small, iterations=1)
...
def process_video(
    ...
    while True:
        ...
        if show_preview:
            ...
            cv2.imshow('Cable Wrap Detection - Real-time', display)
...
def main():
    parser = argparse.ArgumentParser(description='电缆缠绕检测 - SimpleUNet版本')
    ...
    print("="*70)
    print("电缆缠绕均匀性检测（使用SimpleUNet兼容旧模型）")
    print("="*70)
