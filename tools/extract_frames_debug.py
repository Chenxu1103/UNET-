...
def main():
    ...
    print(f"[INFO] 视频 FPS: {src_fps}")
    
    stride = max(1, int(round(src_fps / args.fps)))
    print(f"[INFO] 目标 FPS: {args.fps}, 帧间隔: {stride}")
    ...
    cap.release()
    print(f"[SUCCESS] 保存 {saved} 帧到 {args.out}")
